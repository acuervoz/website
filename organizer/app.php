...
.section-header .pri-asap  { color: var(--accent); }
.section-header .pri-soon  { color: var(--text); }
.section-header .pri-back  { color: var(--text-muted); }
...
.device-label-input:focus { border-color: var(--accent); }
...
.colour-swatch:hover { border-color: var(--text); }
.colour-custom {
  display: flex;
  align-items: center;
  gap: 8px;
  ...
  <div x-show="view==='priority'" x-cloak>
    <template x-for="pri in ['ASAP','Soon','Backlog']" :key="pri">
      <div class="priority-section">
        <div class="section-header" @click="toggleSection(pri)">
            ...
            <div>
              <div class="task-row" :data-id="task.id">
                ...
                <template x-if="editingTask && editingTask.id===task.id">
                  <div class="inline-edit-form" @click.stop>
                    <input type="text" x-model="editingTask.title"
                      ...
                      @blur="saveEdit()" x-init="$el.focus()">
                    <select x-model="editingTask.priority" @change="saveEdit()">
                      <option value="ASAP">!! ASAP</option>
                      <option value="Soon">◈ Soon</option>
                      <option value="Backlog">· Backlog</option>
                    </select>
                    <select x-model="editingTask.project_id" @change="saveEdit()">
                ...
                <div class="task-actions">
                  <button class="task-btn success" @click="openCompleteModal(task)" title="Complete">✓</button>
                  <button class="task-btn" @click="startEdit(task)" title="Edit">✎</button>
                  <div class="dropdown-wrap" @click.outside="closeDropdown(task.id)">
                    <button class="task-btn" @click="toggleDropdown(task.id)">⋮</button>
                    <div class="dropdown-menu" x-show="openDropdownId===task.id" x-cloak @click.stop>
                      ...
                      <template x-for="p in projects.filter(p=>p.id!=task.project_id)" :key="p.id">
                        <button class="dropdown-item" @click="moveTask(task.id, p.id); closeDropdown(task.id)"
                          x-text="'→ '+p.name"></button>
                      </template>
                      <button class="dropdown-item danger" @click="deleteTask(task.id); closeDropdown(task.id)">Delete</button>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <template x-for="sub in task.subtasks" :key="sub.id">
                <div class="task-row subtask">
                  ...
                  <div class="task-actions">
                    <button class="task-btn success" @click="openCompleteModal(sub)" title="Complete">✓</button>
                    <button class="task-btn" @click="startEdit(sub)" title="Edit">✎</button>
                    <div class="dropdown-wrap" @click.outside="closeDropdown(sub.id)">
                      <button class="task-btn" @click="toggleDropdown(sub.id)">⋮</button>
                      <div class="dropdown-menu" x-show="openDropdownId===sub.id" x-cloak @click.stop>
                        ...
                        <button class="dropdown-item danger" @click="deleteTask(sub.id); closeDropdown(sub.id)">Delete</button>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <div class="project-card">
          <div class="project-card-header" style="border-left:3px solid" :style="'border-left-color:'+project.colour">
            ...
            <div class="dropdown-wrap" @click.outside="closeDropdown('proj_'+project.id)" @click.stop>
              <button class="task-btn" style="opacity:0.5" @click.stop="toggleDropdown('proj_'+project.id)">⋮</button>
              <div class="dropdown-menu" x-show="openDropdownId==='proj_'+project.id" x-cloak @click.stop>
                ...
                <button class="dropdown-item danger" @click="deleteProject(project.id); closeDropdown('proj_'+project.id)">Delete</button>
              </div>
            </div>
          </div>
                    ...
                    <template x-for="task in getProjectTasks(project.id, pri)" :key="task.id">
                      <div>
                        <div class="card-task-row">
                          <span class="task-title" x-text="task.title"></span>
                          <div class="task-actions">
                            <button class="task-btn success btn-sm" @click="openCompleteModal(task)" title="Complete">✓</button>
                            <button class="task-btn btn-sm" @click="startEdit(task)" title="Edit">✎</button>
                          </div>
                        </div>
                        <template x-for="sub in task.subtasks||[]" :key="sub.id">
                          <div class="card-task-row subtask">
                            ...
                            <span class="task-title" x-text="sub.title"></span>
                            <div class="task-actions">
                              <button class="task-btn success btn-sm" @click="openCompleteModal(sub)" title="Complete">✓</button>
                            </div>
                          </div>
                        </template>
                      </div>
          ...
          <div class="project-card-footer">
            <template x-if="!addingTaskTo || addingTaskTo !== project.id">
              <button class="btn-text" @click="startAddTask(project.id)">+ ADD TASK</button>
            </template>
  ...
  <div class="modal">
    ...
    <div class="modal-body">
      ...
      <input class="modal-input" type="text" x-model="newProjectName"
        ...
        placeholder="project name..." style="margin-bottom:16px"
        x-init="$nextTick(()=>{$el.focus()})">
              ...
              <td>
                ...
                <template x-if="dev.is_current">
                  <span>
                    ...
                    <span x-text="dev.device_label"></span>
                    <span style="color:var(--text-dim);font-size:10px"> (this device)</span>
                  </span>
                </template>
              </td>
              <td style="color:var(--text-muted)" x-text="relativeTime(dev.last_seen)"></td>
  ...
  <div class="modal" style="min-width:320px">
    ...
    <div class="modal-body">
      ...
      <input class="modal-input" type="text" x-model="labelDeviceValue"
        @keydown.enter="submitLabelDevice()"
...
function app() {
  return {
    ...
    projects: [],
    archivedProjects: [],
    projectTasksCache: {},   // projectId -> [{...task, subtasks:[]}]
    ...
    // ── Data fetching ─────────────────────────────────────────────────────
    async fetchProjects() {
      const data = await this.api('get_projects');
      if (data && !data.error) this.projects = data;
    },

    async fetchArchivedProjects() {
      const data = await this.api('get_archived_projects');
      if (data && !data.error) this.archivedProjects = data;
    },

    async fetchPriorityTasks() {
      const data = await this.api('get_priority_tasks');
      if (data && !data.error) this.priorityTasks = data;
    },

    async fetchAllProjectTasks() {
      const cache = {};
      await Promise.all(this.projects.map(async p => {
        const data = await this.api('get_tasks', { project_id: p.id });
        if (data && !data.error) cache[p.id] = data;
      }));
      this.projectTasksCache = cache;
    },

    async fetchProjectTasks(projectId) {
      const data = await this.api('get_tasks', { project_id: projectId });
      if (data && !data.error) {
        this.projectTasksCache = { ...this.projectTasksCache, [projectId]: data };
      }
    },

    // ── Task helpers ──────────────────────────────────────────────────────
    getProjectTasks(projectId, priority) {
      const tasks = this.projectTasksCache[projectId] || [];
      return tasks.filter(t => t.priority === priority);
    },

    getProjectAllTasks(projectId) {
      return this.projectTasksCache[projectId] || [];
    },
    ...
    async confirmComplete() {
      ...
      const ok = await this.apiPost('complete_task', {
    ...
    async moveTask(id, projectId) {
      ...
      await this.apiPost('update_task', {
        ...
        priority:   task ? task.priority : 'Soon',
    ...
    findTask(id) {
      for (const pri of ['ASAP','Soon','Backlog']) {
        const found = this.priorityTasks[pri].find(t => t.id == id);
        if (found) return found;
        for (const t of this.priorityTasks[pri]) {
    ...
    // ── Add task (dashboard) ──────────────────────────────────────────────
    startAddTask(projectId) {
      this.addingTaskTo   = projectId;
      this.newTaskTitle   = '';
      this.newTaskPriority = 'Soon';
    },

    async submitAddTask(projectId) {
    ...
    async submitRenameProject() {
      ...
      await this.apiPost('update_project', {
        id:     this.editProjectId,
        name:   this.editProjectName.trim(),
        colour: this.editProjectColour,
      });
      this.modal = null;
    ...
    async archiveProject(id) {
      ...
      this.showNotification('project archived', 'info');
      await this.fetchProjects();
      await this.fetchArchivedProjects();
    },
    ...
    async openHistory(taskId) {
      ...
      this.historyTask = task || { title: 'Task #' + taskId };
      this.historyLog  = [];
      this.modal = 'history';
      const data = await this.api('get_task_log', { task_id: taskId });
      if (data && !data.error) this.historyLog = data;
    },

    // ── Devices ───────────────────────────────────────────────────────────
    async openDevices() {
      this.modal = 'devices';
      const data = await this.api('get_devices');
      if (data && !data.error) this.devices = data;
    },
    ...
    async submitLabelDevice() {
      ...
      await this.apiPost('label_device', {
        id:    this.labelDeviceId,
        label: this.labelDeviceValue.trim(),
      });
      this.modal = 'devices';
